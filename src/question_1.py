...
def fetch_all_data():
    data_pipeline('./hockey/test', [2017, 2018, 2019, 2020, 2021], False)
    

def data_pipeline(path_to_directory: str, years_to_fetch: list, override: bool):
    if path.exists(path_to_directory):
        print('File already exists, no download required!')
        return
    
    for year in years_to_fetch:
        download_games_for_season(year, path_to_directory, override)
    return


def download_games_for_season(year: int, path_to_directory, override: bool):
    """
    ...
    try:
        ...
        game_id = build_game_id(year, True)

        #Define variable for the bestOf
        best_of_in_playoff = 7

        #These two could be combined with time
        check_for_directory_existence(path_to_directory+f'/Season{year}{year+1}'+f'/Regular{year}{year+1}') #build the directory path for regular season
        check_for_directory_existence(path_to_directory + f'/Season{year}{year + 1}' + f'/Playoff{year}{year + 1}') #build the directory path for playoff season
        ...
        while(nb_of_miss < threshold_of_miss):
            ...
            play_by_play, status_code = download_play_by_play_for_game_id(game_id)
            if status_code == 200:
                json_to_separate_file(play_by_play, path_to_file, override)
                season_dict = json_to_single_file(play_by_play, game_id, season_dict, path_to_directory+f'/Season{year}{year+1}/season{year}{year+1}.json', override, False)
                nb_of_miss = 0
            else:
                nb_of_miss += 1
            game_id += 1
        ...
        game_id = build_game_id(year, False)
        ...
        while True:
            ...
            if nb_of_miss == best_of_in_playoff:
                ...
                nb_of_matchup += 1
        json_to_single_file('', '', season_dict, path_to_directory + f'/Season{year}{year+1}/season{year}{year+1}.json', override, True)
    except Exception as error:
        print(error)


def build_game_id(year: int, for_regular_season: bool) -> int:
    """
...
def check_for_directory_existence(path_to_directory):
    try:
        if path.exists(path_to_directory) :
...
def json_to_separate_file(json: str, path_to_file: str, override: bool) -> bool:
    """
...
def json_to_single_file(json_game: str, game_id: int, season_dict: dict, path_to_file, override: bool, save_file: bool):
    """
...
def download_play_by_play_for_game_id(game_id: int) -> [str,int]:
    """
